...
read -p "Enter domain or IP for this server [localhost]: " SERVER_HOST
SERVER_HOST=${SERVER_HOST:-localhost}

read -p "Enter a database password [colorpass123]: " DB_PASSWORD
DB_PASSWORD=${DB_PASSWORD:-colorpass123}

read -p "Enter a session secret [autogenerated]: " SESSION_SECRET
SESSION_SECRET=${SESSION_SECRET:-$(openssl rand -hex 32)}

read -p "Enter storage path for incoming files [/data/incoming]: " STORAGE_PATH
STORAGE_PATH=${STORAGE_PATH:-/data/incoming}

read -p "Enter daemon API key for site daemons [autogenerated]: " DAEMON_API_KEY
...
echo ""
echo "Step 13: Seeding initial data..."
curl -s -X POST http://localhost:5000/api/seed || echo "Seed request sent"
...
echo ""
echo "Other seeded users (all have password 'admin123'):"
echo "  - jsmith (colorist)"
echo "  - mwilson (colorist)"
echo ""
...
echo "  2. Install daemons at each site with the API key shown above"
echo "  3. Change default passwords for all users"
echo ""
